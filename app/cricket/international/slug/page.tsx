...
export async function generateStaticParams() {
  const slugs = getAllSlugs('cricket', 'international')
  return slugs.map((slug) => ({ slug }))
}

export default function InternationalArticlePage({ params }: { params: { slug: string } }) {
  const article = getArticleBySlug('cricket', 'international', params.slug)
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <nav className="mb-6 text-sm text-gray-600">
        ...
        <span className="mx-2">›</span>
        <Link href="/cricket/international" className="hover:text-blue-600">International</Link>
        <span className="mx-2">›</span>
      ...
      {/* Back Button */}
      <div className="mt-8">
        <Link href="/cricket/international" className="text-blue-600 hover:underline">
          ← Back to all International Cricket predictions
        </Link>
      </div>
    </div>
  )
